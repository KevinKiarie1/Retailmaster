...
import { StatsCard, RevenueChart, KeyStats } from '@/components';
import { dashboardAPI } from '@/services/api';
import { DashboardStats, RevenueTrendItem } from '@/types';
...
export default function Dashboard() {
  ...
  const [revenueTrend, setRevenueTrend] = useState<RevenueTrendItem[]>(defaultRevenueTrend);
  ...
  const fetchDashboardData = async (): Promise<void> => {
    try {
      ...
      if (trendRes.data?.success && trendRes.data.data && trendRes.data.data.length > 0) {
